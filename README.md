...
7. Нажми **Запустить ViaProxy** → **Запустить бота** (или кнопку **Запустить всё**).
...
- Автоподключение при разрыве (ECONNRESET).
- Показ причин ошибок запуска в панели.
- Кнопка «Запустить всё» (ViaProxy + бот).
- Ожидание готовности порта перед запуском бота.
